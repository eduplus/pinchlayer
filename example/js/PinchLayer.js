(function() {
	...
	Kinetic.PinchLayer.prototype.dragStarted = function(event) {
	 	if (!event.touches || event.touches.length <= 1) {
		 	self.lastPosition.x = (event.clientX || self.getParent().getPointerPosition().x);
		 	self.lastPosition.y = (event.clientY || self.getParent().getPointerPosition().y);
		}
	}

	Kinetic.PinchLayer.prototype.dragMoved = function(event) {
	 	if (!event.touches || event.touches.length <= 1) {
		 	var calcDeltaX = (event.clientX || self.getParent().getPointerPosition().x) - self.lastPosition.x;
		 	var calcDeltaY = (event.clientY || self.getParent().getPointerPosition().y) - self.lastPosition.y;
		 	self.lastPosition.x = (event.clientX || self.getParent().getPointerPosition().x);
		 	self.lastPosition.y = (event.clientY || self.getParent().getPointerPosition().y);
